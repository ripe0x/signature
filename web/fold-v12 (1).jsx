...
function generateCellDimensions(width, height, padding, seed) {
  ...
  const validWidths = getDivisors(innerW, 4, 36);
  const validHeights = getDivisors(innerH, 4, 50);
  ...
  if (sizeBias < 0.25) {
    ...
    cellW = validWidths[0];
    cellH = validHeights[Math.floor(rng() * Math.min(3, validHeights.length))];
  } else if (sizeBias > 0.75) {
    // Large cells (coarse/chunky)
    cellW = validWidths[validWidths.length - 1];
    cellH =
      validHeights[
        Math.max(0, validHeights.length - 1 - Math.floor(rng() * 3))
      ];
  } else {
    // Random from full range
    cellW = validWidths[Math.floor(rng() * validWidths.length)];
    cellH = validHeights[Math.floor(rng() * validHeights.length)];
  }
...
// Process creases with intersection-based threshold removal
// Each crease has a threshold - if it would hit a cell that already has >= threshold weight, it dies
...
function processCreasesWithThreshold(
  ...
  // Sort by depth (earlier creases processed first)
  const sortedCreases = [...creases].sort((a, b) => a.depth - b.depth);

  for (const crease of sortedCreases) {
    // Each crease has its own threshold (50% to 150% of base)
    const creaseRng = seededRandom(seed + crease.depth * 7919);
    const thresholdMultiplier = 0.5 + creaseRng() * 1.0;
    const creaseThreshold = Math.floor(baseThreshold * thresholdMultiplier);

    // Find intersections with already-active creases
    let hitSaturatedCell = false;
    const newIntersections = [];

    for (const otherCrease of activeCreases) {
      const hit = segmentIntersect(
        crease.p1,
        crease.p2,
        otherCrease.p1,
      ...
      if (hit) {
        ...
        if (col >= 0 && col < gridCols && row >= 0 && row < gridRows) {
          const key = `${col},${row}`;
          const currentWeight = cellWeights[key] || 0;

          // Check if this cell has reached the crease's threshold
          if (currentWeight >= creaseThreshold) {
            hitSaturatedCell = true;
            break;
          }

          // Calculate intersection data
          const weight1 = crease.weight || 1;
          const weight2 = otherCrease.weight || 1;
          const gap = Math.abs(otherCrease.depth - crease.depth);

          newIntersections.push({
            x: hit.point.x,
            y: hit.point.y,
            depth1: crease.depth,
            depth2: otherCrease.depth,
            gap: gap,
            weight: weight1 + weight2,
            key: key,
          });
    ...
    // If crease didn't hit a saturated cell, add it
    if (!hitSaturatedCell) {
      activeCreases.push(crease);

      // Add intersections and update cell weights/gaps
...
function renderToCanvas({
  ...
  const baseThreshold = Math.floor(3 + destructionThreshold * 17); // 3-20 intersections base
          ...
          >
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCellWidth(
                    Math.max(4, Math.min(32, parseInt(e.target.value) || 8))
                  )
                }
                style={{
                  width: 40,
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCellHeight(
                    Math.max(4, Math.min(32, parseInt(e.target.value) || 12))
                  )
                }
                style={{
                  width: 40,
